...
double earthRopeGap(double earthRadiusKm) {
    ...
    const double newLength = initialLength + 1.0;   // +1 метр
...
std::pair<double, double> poolCosts(double poolRadius, double pathWidth,
    ...
    const double pathArea = M_PI * (outerRadius * outerRadius - poolRadius * poolRadius);
    const double concreteCost = pathArea * concreteCostPerSqM;
    const double fenceLength = 2.0 * M_PI * outerRadius;
    const double fenceCost = fenceLength * fenceCostPerM;
